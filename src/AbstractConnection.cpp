...
bool AbstractConnection::do_write(const QByteArray & data)
{
    ...
    if (writeBackLog > MAX_BUFFER) {
        qWarning(f) << prettyName() << "-- MAX_BUFFER reached on write (" << MAX_BUFFER << "), disconnecting client";
        do_disconnect();
        return false;
    }
    ...
    if (UNLIKELY(written < 0)) {
        qCritical(f) << prettyName() << "-- error on write" << socket->error() << "(" << socket->errorString() << ")";
        do_disconnect();
        return false;
        ...
        qCritical(f) <<  prettyName() << "-- short write count; expected to write" << n2write
...
void AbstractConnection::on_connected()
{
    ...
    connectedConns.push_back(
        connect(socket, &QAbstractSocket::disconnected, this, [this]{
            qCDebug(f) << prettyName() << "socket disconnected";
...
{
    qCDebug(f) << prettyName() << "socket state:" << int(s);
    switch (s) {
...
void AbstractConnection::on_bytesWritten(qint64 nBytes)
{
    ...
    if (writeBackLog > 0 && status == Connected && socket) {
        qCDebug(f) << prettyName() << "writeBackLog size:" << writeBackLog << " (wrote just now: " << nBytes << ")";
    }
}

void AbstractConnection::do_ping()
{
    qCDebug(f) <<  prettyName() << "stub ...";
}

void AbstractConnection::on_error(QAbstractSocket::SocketError err)
{
    qCWarning(f) << prettyName() << ": error" << err << "(" << (lastSocketError = (socket ? socket->errorString() : "(null)")) << ")";
    ++nSocketErrors;
    do_disconnect();
}
