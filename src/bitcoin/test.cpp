...
namespace bitcoin {
    bool TestBase58(bool silent, bool throws)
    {
        ...
        if (!silent) qDebug() << ba.toHex() << "  ->  " << str.c_str() << "  (encoded)";
        bool ret = ba.toHex() == hexDecoded && str == std::string(anAddress);
        if (!silent) qDebug() << (ret ? "Compare ok, success." : "Values differ -- ERROR!!");
        return ret;
    }
}
