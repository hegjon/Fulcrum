...
namespace Merkle {
    ...
    BranchAndRootPair branchAndRoot(const HashVec &hashVec, unsigned index, const std::optional<unsigned> & optLen)
    {
        ...
        if (!hvsz || index >= hvsz) {
            qCritical() << __PRETTY_FUNCTION__ << ": Misused. Please specify a non-empty hash vector as well as an in-range index. FIXME!";
        ...
        if (length < natLen) {
            qCritical() << __PRETTY_FUNCTION__ << ": Misused. Must specify a length argument that is >= " << natLen << " for a vector of size " << hvsz << ". FIXME!";
        ...
        if (UNLIKELY(hashes.empty())) {
            qCritical() << __PRETTY_FUNCTION__ << ": INTERNAL ERROR. Output vector is empty! FIXME!";
    ...
    Hash rootFromProof(const Hash & hashIn, const HashVec &branch, unsigned index)
    {
        ...
        if (index) {
            qCritical() << __PRETTY_FUNCTION__ << ": INTERNAL ERROR. Passed-in index is out of range! FIXME!";
    ...
    HashVec level(const HashVec &hashes, unsigned depthHigher)
    {
        ...
        if (depthHigher > MaxDepth) {
            qCritical() << __PRETTY_FUNCTION__ << ": INTERNAL ERROR. depthHigher is too large " << depthHigher << " > " << MaxDepth << ". FIXME!";
            throw BadArgs("Argument depthHigher is too large");
        }
        if (hashes.empty()) {
            qCritical() << __PRETTY_FUNCTION__ << ": INTERNAL ERROR. empty hashes vector! FIXME!";
    ...
    BranchAndRootPair branchAndRootFromLevel(const HashVec & level, const HashVec & leafHashes, unsigned index, unsigned depthHigher)
    {
        ...
        if (level.empty() || leafHashes.empty() || depthHigher > MaxDepth) {
            qCritical() << __PRETTY_FUNCTION__ << ": Invalid args";
        ...
        if (index >= level.size() || leafRoot != level[index]) {
            qCritical() << __PRETTY_FUNCTION__ << ": leaf hashes inconsistent with level. FIXME!";
