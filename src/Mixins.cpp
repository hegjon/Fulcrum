...
QObject *QObjectMixin::qobj() const
{
    ...
    if (!ret) {
        qCritical(f) << __PRETTY_FUNCTION__ << ": Cannot cast this to QObject! App will likely crash now!";
    }
    return ret;
}
...
auto StatsMixin::statsSafe(int timeout_ms) const -> Stats
{
    ...
    try {
        ...
        qDebug(f) << "Safe stats get failed:" <<  e.what();
...
auto StatsMixin::debugSafe(const StatsParams &p, int timeout_ms) const -> Stats
{
    ...
    try {
        ...
        qDebug(f) << "Safe debug get failed:" << e.what();
        ret = QVariantMap{{"error" , e.what()}};
    }
    return ret;
}
