            ...
            if (LIKELY(!ret)) {
                ...
                ret = DataRef(new Data, [weakThis = QPointer(this), key, myname = objectName()](Data *p){
                    ...
                    constexpr auto MkDeleteFunc = [](Data *&p, const auto & myname, const auto & key) constexpr -> auto
                    {
                        ...
                        if constexpr (debugPrt)
                            return [&p, &myname, &key] {
                                qCDebug(f) << myname << ": entry for " << ToString(key) << " DELETED!";
                                delete p; p = nullptr;
                            };
                        else
                    ...
                    if (UNLIKELY(me->isDeleted)) {
                        if constexpr (debugPrt)
                            qCDebug(f) << myname << ": is cleaing up, skipping remove-from-table for " << ToString(key);
                        return;
                    }
                    ...
                    if (LIKELY(it != me->table.end())) {
                        if (const auto existingRef = it.value().lock();
                            ...
                            if constexpr (debugPrt)
                                qCDebug(f) << myname << ": Removed entry from table for " << ToString(key) << ", table-size now " << me->table.size();
                        }
                    } else {
                ...
                if constexpr (debugPrt)
                    qCDebug(f) << objectName() << ": entry for " << ToString(key) << " CREATED!";
            } else
