...
public:
    ...
    ~ThreadSafeHashTable() override {
        ...
        if constexpr (debugPrt) {
            qCDebug(normal) << objectName() << ": Checking table...";
    ...
    DataRef getOrCreate(const Key &key, bool createIfMissing)
    {
        ...
        constexpr auto MkDebugPrtFunc = [](const auto & key, const auto & ret, auto me) constexpr -> auto  {
            ...
            if constexpr (debugPrt)
                return [&key, &ret, me] {
                    qCDebug(normal) << me->objectName() << ": " << ToString(key) << " found existing with refct: " << ret.use_count();
                };
            else return [] {};
        };
            ...
            if (LIKELY(!ret)) {
                ...
                ret = DataRef(new Data, [weakThis = QPointer(this), key, myname = objectName()](Data *p){
                    ...
                    constexpr auto MkDeleteFunc = [](Data *&p, const auto & myname, const auto & key) constexpr -> auto
                    {
                        ...
                        if constexpr (debugPrt)
                            return [&p, &myname, &key] {
                                qCDebug(normal) << myname << ": entry for " << ToString(key) << " DELETED!";
                                delete p; p = nullptr;
                            };
                        else
                    ...
                    if (UNLIKELY(me->isDeleted)) {
                        if constexpr (debugPrt)
                            qCDebug(normal) << myname << ": is cleaing up, skipping remove-from-table for " << ToString(key);
                        return;
                    }
                    ...
                    if (LIKELY(it != me->table.end())) {
                        if (const auto existingRef = it.value().lock();
                            ...
                            if constexpr (debugPrt)
                                qCDebug(normal) << myname << ": Removed entry from table for " << ToString(key) << ", table-size now " << me->table.size();
                        }
                    } else {
                ...
                if constexpr (debugPrt)
                    qCDebug(normal) << objectName() << ": entry for " << ToString(key) << " CREATED!";
            } else
