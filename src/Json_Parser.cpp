...
#   define clearExpect(bit) (expectMask &= ~ExpectBits::EXP_##bit)
    ...
    do {
            ...
            if (stack.empty()) {
                ...
                if (top->typ == VType::Obj) {
                    ...
                    if (UNLIKELY(top->entries.empty())) {
                        qCCritical(f) << "Json Parser ERROR: Obj 'entries' is empty; FIXME!";
                        return false;
                    }
                    // /paranoia
        ...
        case JTOK_KW_FALSE: {
            ...
            if (top->typ == VType::Obj) {
                // paranoia
                if (UNLIKELY(top->entries.empty())) {
                    qCCritical(f) << "Json Parser ERROR: Obj 'entries' is empty when parsing a keyword; FIXME!";
                    return false;
                }
                // /paranoia
        ...
        case JTOK_NUMBER: {
            ...
            if (top->typ == VType::Obj) {
                // paranoia
                if (UNLIKELY(top->entries.empty())) {
                    qCCritical(f) << "Json Parser ERROR: Obj 'entries' is empty when parsing a number; FIXME!";
                    return false;
                }
                // /paranoia
                top->entries.back().second = std::move(tmpVal);
        ...
        case JTOK_STRING: {
            if (expect(OBJ_NAME)) {
                ...
                if (top->typ == VType::Obj) {
                    // paranoia
                    if (UNLIKELY(top->entries.empty())) {
                        qCCritical(f) << "Json Parser ERROR: Obj 'entries' is empty when parsing a string; FIXME!";
                        return false;
                    }
                    // /paranoia
                    top->entries.back().second = std::move(tmpVal);
    ...
    try {
        ...
        qCWarning(f) << "Failed to parse JSON: " << e.what();
        return false;
    }
